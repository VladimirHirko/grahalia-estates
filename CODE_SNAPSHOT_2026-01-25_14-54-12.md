# Code snapshot (2026-01-25T14:54:12)
Всего файлов: 122
...
.gallery {
  ...
  align-items: start; /* ✅ важно: убирает растягивание coverWrap */
}
...
.thumbs {
  ...
  max-height: 420px;      /* ✅ высота как у cover */
  overflow-y: auto;       /* ✅ прокрутка */
  padding-right: 4px;     /* чтобы скролл не прилипал */
}
...
/* Кнопка "Filters" — используем глобальный .btn btnGhost,
   тут только компактность и внутренние элементы */
.filtersToggleBtn {
  display: inline-flex;
  align-items: center;
  gap: 10px;
...
/* панель фильтров (вместо огромной сетки) */
.filtersPanel {
  margin-top: 12px;
  border: 1px solid var(--border);
...
/* Вместо гигантских чекбоксов — pills */
.featuresCompact {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
...
## app/admin/properties/new/RentFieldsToggle.tsx
...
.seg .btn {
  border: 0;              /* граница у контейнера */
  border-radius: 0;       /* чтобы не ломать pill */
  padding: 10px 14px;
  min-height: 38px;
}
...
function isNewBuildFromCondition(v: unknown): boolean {
  ...
  // "new_build", "new build", "newbuild", "New build" и т.п.
  if (!s) return false;
...
export async function getFeaturedItems(lang: "en" | "es", limit = 6): Promise<FeaturedItem[]> {
  // 1) Published properties (include rent fields + condition)
  ...
  // ✅ ВАЖНО: используем IN (...) (не ANY), чтобы не ловить cast record -> int[]
  ...
  return props.map((p) => {
    ...
    return {
      ...
      isNewBuild, // ✅ вот это и нужно для бейджа на главной
    };
  });
}
...
export async function getCatalogItems(opts: {
  ...
  const items: CatalogItem[] = ids.map((id) => {
    ...
    const condition = String(p?.condition ?? "").trim().toLowerCase();
    const isNewBuild =
      condition === "new_build" || condition === "newbuild" || condition === "new";
    ...
    return {
      ...
      // ✅ теперь поле реально заполняется
      isNewBuild,
    };
  });
...
/* мобилка: меню потом сделаем бургером */
...
.row {
  ...
  gap: 12px;
  align-items: end; /* ✅ выравниваем колонки по нижнему краю */
}
...
.field {
  ...
  flex-direction: column;
  margin-top: 12px;
  gap: 0; /* ✅ чтобы отступ задавал label */
}
...
.label {
  display: block;
  min-height: 16px;   /* ✅ одинаковая высота лейбла */
  line-height: 16px;  /* ✅ чтобы нигде не "плавало" */
  font-size: 12px;
  color: rgba(28, 28, 28, 0.75);
  font-weight: 600;
  margin-bottom: 6px; /* ✅ фиксируем отступ, вместо gap */
}
...
.input:focus,
.textarea:focus {
  border-color: rgba(43, 69, 55, 0.45); /* под твой --accent */
...
/* hover = только смена цвета, без transform */
.form :global(.btn.btnPrimary:hover) {
  background: var(--accentHover) !important;
  transform: none !important;
}

/* active оставляем "тап" как в глобале */
...
.infoIcon {
  ...
  display: grid;
  place-items: center;
  background: rgba(43, 69, 55, 0.08); /* под --accent */
}
...
export type FeaturedItem = {
  ...
  title: string;
  location: string;
  price: string; // already formatted (sale OR rent)
  beds: number;
  baths: number;
  area: number;
  image: string;
...
.location {
  margin-top: -2px;      /* ✅ подтягиваем ближе к логотипу */
  font-size: 13px;
  color: rgba(28, 28, 28, 0.6);
  text-align: center;
  letter-spacing: 0.2px;
...
@media (max-width: 519px) {
  ...
  /* Brand — на всю ширину и по центру */
  .col:first-child {
    grid-column: 1 / -1;
    display: grid;
    justify-items: center;
  ...
  /* Убираем текстовый слоган, если он ещё где-то был */
  .tagline {
    display: none;
  }

  /* Costa del Sol • Spain под логотипом, ближе */
  .meta {
    margin-top: 8px;
    text-align: center;
  }
  ...
  /* Manage cookies — в той же группе, но не огромной полосой */
  .manageBtn {
...
/* Wider mobile / small tablets: твой вариант “3 зоны в одну строку” */
...
.logo {
  ...
  align-items: center;
  cursor: pointer;
  line-height: 0; /* убирает лишнюю высоту */
}

/* сам логотип-картинка */
.logoImg {
  height: 75px;   /* ✅ высота логотипа в хедере */
  width: auto;
  display: block;
  object-fit: contain;
}
...
.card {
  ...
  display: flex;
  flex-direction: column;
  height: 100%; /* ✅ важно: карточка тянется по высоте ячейки */
...
.body {
  ...
  /* ✅ ключевой фикс: тело карточки управляет вертикальной раскладкой */
  display: flex;
  flex-direction: column;
  flex: 1; /* занимает всё оставшееся место */
  min-height: 0;

  /* ✅ добавляем "ритм" между блоками */
  gap: 6px;
}

.spacer {
  flex: 1;
  min-height: 14px; /* ✅ гарантированный "воздух" перед кнопкой */
}
...
.btn {
  /* ✅ но мы гарантируем "воздух" над кнопкой */
  padding-top: 14px;
...
/* -------------------------------------------------
   ...
   New build — слева сверху
-------------------------------------------------- */

/* Оставляем (на случай если где-то ещё используется), но в новом варианте не обязателен */
.badgeStack {
  position: absolute;
  top: 12px;
  right: 12px;
...
/* ✅ For Sale / For Rent (по умолчанию справа сверху) */
.dealBadge {
  position: absolute;
  top: 12px;
  right: 12px;
...
/* ✅ New build (тот же стиль, но уезжает влево и чуть “теплее”) */
.newBuildBadge {
  top: 12px;
  left: 12px;
  right: auto;

  /* визуально чуть отличается, но без «шока» */
  padding: 5px 10px;
  font-size: 11px;
  font-weight: 700;

  background: rgba(234, 244, 238, 0.92);
  border: 1px solid rgba(207, 227, 216, 1);
  color: rgba(47, 107, 79, 1);
}

/* ✅ debug-бейдж оставляем как был (если используешь) */
.dealBadgeDebug {
  position: absolute;
  top: 12px;
  left: 12px;
...
export default function PropertyCard({
  ...
  const dealLabel =
    dealType === "sale" ? t.forSale : dealType === "rent" ? t.forRent : null;
  ...
  return (
    <article className={styles.card}>
      <div className={styles.imageWrap}>
        ...
        {/* ✅ New build — слева сверху */}
        {newBuildLabel ? (
          <span className={`${styles.dealBadge} ${styles.newBuildBadge}`} aria-label={newBuildLabel}>
            {newBuildLabel}
          </span>
        ) : null}

        {/* ✅ DEV-подсказка: если нет dealType — увидишь сразу */}
...
.thumbs {
  ...
  /* ✅ ключевое: ограничиваем высоту правой колонки и включаем скролл */
  height: var(--cover-h);
  overflow-y: auto;
  overflow-x: hidden;
  ...
  /* чтобы скролл не "съедал" ширину и сетка не прыгала */
...
@media (max-width: 720px) {
  ...
  .thumbs {
    height: auto;                 /* ✅ убираем десктоп-ограничение */
    overflow-y: hidden;
    overflow-x: auto;
...
```json
{
  ...
  "contact": {
    ...
    "form": {
      ...
      "emailPlaceholder": "you@email.com",
      "messagePlaceholder": "Tell us what you are looking for...",
      "messageHint": "Tip: please enter at least 15 characters (more details = faster response).",
      "messageMinError": "Please enter at least 15 characters in the message."
    },
    "info": {
      "title": "Contact",
  ...
  "footer": {
    ...
    "locationLine": "Costa del Sol • Spain",
    "navTitle": "Links",
    "nav": {
      "home": "Home",
      "properties": "Properties",
...
```json
{
  ...
  "hero": {
    "titleA": "Encuentra la casa de tus sueños",
    "titleB": "en España",
  ...
  "featured": {
    ...
    "subtitle": "Descubre nuestras últimas ofertas",
  ...
  "services": {
    ...
    "items": [
      ...
      {
        "title": "Asesoramiento en inversión",
        "text": "Consejos expertos para inversiones rentables.",
        "icon": "📈"
      },
      {
        "title": "Asistencia legal",
        "text": "Apoyo completo con trámites y documentación.",
        "icon": "📄"
      }
    ]
  },
  "about": {
    ...
    "subtitle": "Tu socio inmobiliario de confianza en España",
    "body": "Ofrecemos un servicio excepcional y un profundo conocimiento local para ayudarte a encontrar la propiedad ideal en la Costa del Sol. Desde la elección de la zona adecuada hasta la negociación y los trámites legales, te acompañamos en cada paso con claridad y profesionalidad.",
    "cta": "Saber más"
  },
  "contact": {
    "title": "Ponte en contacto",
    "subtitle": "¿Listo para encontrar la casa de tus sueños?",
    "form": {
      "name": "Tu nombre",
      "phone": "Teléfono",
      ...
      "messageHint": "Consejo: introduce al menos 15 caracteres (más detalles = respuesta más rápida).",
      "messageMinError": "Introduce al menos 15 caracteres en el mensaje."
    },
    "info": {
      "title": "Contacto",
      "whatsapp": "Escríbenos por WhatsApp",
      "note": "Normalmente respondemos en pocas horas durante días laborables."
    }
  },

  "footer": {
    "tagline": "Tu hogar, con claridad",
    "locationLine": "Costa del Sol • España",
    "navTitle": "Enlaces",
    "nav": {
      "home": "Inicio",
      "properties": "Propiedades",
  ...
  "cookies": {
    "consent": {
      ...
      "modalText": "Elige qué cookies permites. Las cookies necesarias siempre están activadas.",

      "necessaryTitle": "Necesarias",
      "necessaryDesc": "Son imprescindibles para que el sitio funcione correctamente.",

      "analyticsTitle": "Analítica",
      "analyticsDesc": "Nos ayuda a entender cómo se utiliza el sitio.",

      "marketingTitle": "Marketing",
      "marketingDesc": "Nos ayuda a mostrar anuncios relevantes y medir campañas.",
