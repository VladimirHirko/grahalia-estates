# Code snapshot (2026-01-24T22:11:35)
Всего файлов: 121
  ...
  const [featuresForFilters, result] = await Promise.all([
    ...
    getCatalogItems({
      ...
      filters: {
        ...
        // ✅ NEW: пробрасываем deal (sale/rent/all)
        deal,
      },
    }),
  ]);
              ...
              <>
                ...
                <div className={styles.pager}>
                  <div className={styles.pagerInfo}>
                    {lang === "es" ? `Página ${result.page} de ${result.totalPages}` : `Page ${result.page} of ${result.totalPages}`}
                  </div>
...
export async function getFeaturedItems(
  ...
  // 1) Published properties (include rent fields)
  ...
  return props.map((p) => {
    ...
    const dealType = p.deal_type ? String(p.deal_type) : null;

    const price =
      p.price !== null && p.price !== undefined ? Number(p.price) : null;

    const rentPrice =
      p.rent_price !== null && p.rent_price !== undefined
        ? Number(p.rent_price)
        : null;
    ...
    return {
      ...
      price: formatDisplayPrice({
        lang,
        dealType,
        price, // ✅ ВАЖНО: price, не salePrice
        currency,
        rentPrice,
        rentPeriod,
      }),
  ...
  const items: CatalogItem[] = ids.map((id) => {
    ...
    return {
      ...
      priceText: formatDisplayPrice({
        lang,
        dealType,
        price: salePrice, // ✅ важно: тут "price", не "salePrice"
        currency,
        rentPrice,
        rentPeriod,
      }),
...
export default function Featured({
  ...
  // Keep page in range when pool/pageCount changes
  useEffect(() => {
    setPage((p) => (p >= pageCount ? 0 : p));
  }, [pageCount]);

  // Optional random start (only once per pool length change)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // goNext uses pageCount/fadeMs via closure, but we deliberately reset interval on these deps:
  ...
  const visible = useMemo(() => {
    ...
    // loop-fill if we're at the end and need more items
...
export default function LeadContactForm({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // ✅ source определяем автоматически
    const source = propertySlug ? "property" : "contact";

    // ✅ ВАЖНО: берём актуальный URL В МОМЕНТ ОТПРАВКИ
    const currentUrl =
...
export default function LeadForm({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // ✅ ВАЖНО: актуальный URL в момент отправки
    const currentUrl =
      typeof window !== "undefined" && window.location?.href
        ? window.location.href
        : resolvedPageUrl || "/";
    ...
    <section className={styles.wrap} aria-label="Lead form">
      ...
      <form className={styles.form} onSubmit={onSubmit}>
        ...
        <div className={styles.actions}>
          ...
          {ok && <span className={styles.ok}>Sent ✅</span>}
