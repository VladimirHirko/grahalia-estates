# Code snapshot (2026-01-18T13:53:55)
Всего файлов: 80
...
export default async function PropertiesPage({
  ...
  // ✅ Берём много объектов для каталога (позже будет пагинация)
  const items = await getFeaturedItems(lang === "es" ? "es" : "en", 48);
          ...
          <div className="container">
            ...
            {items.length === 0 ? (
              <div className={styles.empty}>
                {lang === "es" ? "No hay propiedades todavía." : "No properties yet."}
              </div>
            ) : (
    ...
    <section id="properties" className={`section ${styles.section}`}>
      <div className="container">
        ...
        >
          <div className={styles.grid}>
            {visible.map((p) => (
              <PropertyCard key={String(p.id ?? p.title)} property={p} t={t.card} />
            ))}
          </div>
        </div>
...
export default function Header({ lang, t }: HeaderProps) {
  ...
  const goTo = (id: string) => {
    ...
    // На других страницах (каталог) — уводим на главную с hash
    router.push(`/${lang}#${id}`);
  };
